-- Lua Unit was lightly modified by me (Nayruden)
...
assert_tables_equal = AssertTablesEqual
    ...
    function LuaUnit:runTestClassByName( aClassName )
		...
		if hasMethod then
			...
			--for methodName, method in classInstance do
				if LuaUnit.isFunction(method) and string.sub(methodName, 1, 4):lower() == "test" then
					LuaUnit:runTestMethodName( aClassName..':'.. methodName, classInstance )
				end
			end
		end
